<?php
require_once('header.php');

$breadcrumb = ['關於鼎億','公司介紹'];
require_once('breadcrumb.php');
?>
<!-- title start -->
<section>
    <h5 class="text-center">公司介紹</h5>
    <h6 class="subtitle text-center">About</h6>
</section>
<!-- title end -->
<?php
$sidebar = [
    'title' => '關於鼎億',
    'items' => ['最新消息','公司介紹','發展沿革','理念/使命/願景'],
    'active' => 2
];
require_once('wrapper.php');
?>
<img src="image/page/company/1.png" class="img-fluid" alt="Company Banner 1">
<h3 class="text-center">公司簡介</h3>
<p>鼎億(廈門)貿易有限公司，是台灣鼎億有限公司於廈門成立的服務據點，2010年元月正式營運。具有獨立法人營業資格及合法進出口執照，提供廣大中國客戶更便捷的產品購買及技術支持。</p>
<p>鼎億(廈門)貿易有限公司，是台灣鼎億有限公司於廈門成立的服務據點，2010年元月正式營運。具有獨立法人營業資格及合法進出口執照，提供廣大中國客戶更便捷的產品購買及技術支持。</p>
<h3 class="text-center">為什麼選擇鼎億</h3>
<p>鼎億(廈門)貿易有限公司，是台灣鼎億有限公司於廈門成立的服務據點，2010年元月正式營運。具有獨立法人營業資格及合法進出口執照，提供廣大中國客戶更便捷的產品購買及技術支持。</p>
<h3 class="text-center">經驗和專業</h3>
<p>鼎億(廈門)貿易有限公司，是台灣鼎億有限公司於廈門成立的服務據點，2010年元月正式營運。具有獨立法人營業資格及合法進出口執照，提供廣大中國客戶更便捷的產品購買及技術支持。</p>
<p>鼎億(廈門)貿易有限公司，是台灣鼎億有限公司於廈門成立的服務據點，2010年元月正式營運。具有獨立法人營業資格及合法進出口執照，提供廣大中國客戶更便捷的產品購買及技術支持。</p>
<img src="image/page/company/2.png" class="img-fluid" alt="Company Banner 2">
<?php
require_once('footer.php');
?>